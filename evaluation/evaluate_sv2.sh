...
for sample in ${samples[@]}; do
    ...
    bcftools view -i 'GT!="0/0" & SVTYPE="DEL"'\
        $data_dir/$sample/VCF/SV2/sv2_genotypes/${sample}_sv2.vcf |
    grep -v contig |
    bcftools annotate -h $data_dir/$sample/VCF/header_contigs.txt |
    sed 's/##INFO=<ID=GENES,Number=1/##INFO=<ID=GENES,Number=./g' |
    bgzip -c > $data_dir/$sample/VCF/SV2/sv2_genotypes/temp.vcf.gz
    tabix $data_dir/$sample/VCF/SV2/sv2_genotypes/temp.vcf.gz

    bash truvari.sh \
        -c $data_dir/$sample/VCF/SV2/sv2_genotypes/temp.vcf.gz \
        -b $data_dir/$sample/VCF/$truth_set \
        -o $data_dir/$sample/VCF/SV2/truvari
