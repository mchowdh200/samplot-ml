...
function call_and_genotype {
    ...
    [[ ! -d $data_dir/$sample/VCF/SV2-filter ]] && mkdir $data_dir/$sample/VCF/SV2-filter
    ...
    sv2 -g $genome \
        -i $bam \
        -v $data_dir/$sample/VCF/smoove/$sample-smoove.genotyped.del.vcf.gz \
        -snv $snv_vcf \
        -p $data_dir/$sample/VCF/$sample.ped \
        -odir $data_dir/$sample/VCF/SV2-filter \
        -o ${sample}_smoove_sv2
}
