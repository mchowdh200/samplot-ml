...
function call_and_genotype {
    ...
    [[ ! -d $data_dir/$sample/VCF/SV2-filter-manta ]] && mkdir $data_dir/$sample/VCF/SV2-filter-manta
    ...
    sv2 -g $genome \
        -i $bam \
        -v $data_dir/$sample/VCF/manta/results/variants/diploidSV.del.duphold.vcf.gz \
        -snv $snv_vcf \
        -p $data_dir/$sample/VCF/$sample.ped \
        -odir $data_dir/$sample/VCF/SV2-filter-manta \
        -o ${sample}_manta_sv2
}
